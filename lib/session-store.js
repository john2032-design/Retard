...
export async function getPublicKey(apiKey) {
  const client = getClient();
  const raw = await client.get(`pubkey:${apiKey}`);
  return raw || null;
}

export async function setPublicKey(apiKey, pubKey) {
  const client = getClient();
  await client.set(`pubkey:${apiKey}`, pubKey);
}
...
export async function deleteUsedToken(token) {
  const client = getClient();
  await client.del(`used:${token}`);
}

export async function storeNonce(nonce, apiKey) {
  const client = getClient();
  await client.set(`nonce:${nonce}`, apiKey, "EX", 30);
}

export async function validateAndConsumeNonce(nonce, apiKey) {
  const client = getClient();
  const storedApiKey = await client.get(`nonce:${nonce}`);
  if (!storedApiKey) return false;
  if (storedApiKey !== apiKey) return false;
